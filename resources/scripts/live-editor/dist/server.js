...
var formatArgs = (args, colors = true) => args.map((e) => typeof e === "string" ? e : import_util.default.inspect(e, {
  ...inspectSettings,
  colors
})).join(" ");
var patchAlt = (player, id) => {
  return {
    ...alt,
    log: (...args) => {
      alt.log(...args);
...
alt.onClient("codeEditor:eval", async (player, promiseId, code, id) => {
  try {
    const res = await new AsyncFunction("alt", "player", "console", ...Object.keys(codeHelpers), code)(patchAlt(player, id), player, patchConsole(player, id), ...Object.values(codeHelpers));
    return alt.emitClient(player, "$repl", promiseId, import_util.default.inspect(res, inspectSettings));
  } catch (e) {
    if (e instanceof Error) {
      return alt.emitClient(player, "$repl", promiseId, colorizeError(String(e.stack)));
    }
    return alt.emitClient(player, "$repl", promiseId, colorizeError(import_util.default.inspect(e, { ...inspectSettings, colors: false })));
  }
});
alt.onClient("qaTools:spawn", (player, vector) => {
  player.spawn(vector);
});
alt.onClient("qaTools:fly", (player, state) => {
